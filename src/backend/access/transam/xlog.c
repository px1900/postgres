...
ssize_t pg_pwritev_rpc_local(int fd, const struct iovec *iov, int iovcnt, off_t offset) {
	if(IsRpcClient) {
		// For loop, call RpcPgPwrite for each iov
		ssize_t ret = 0;
		for (int i = 0; i < iovcnt; i++) {
			ret += RpcPgPWrite(fd, iov[i].iov_base, iov[i].iov_len, offset);
			offset += iov[i].iov_len;
		}

	} else {
		return pg_pwritev_with_retry(fd, iov, iovcnt, offset);
	}
...
#ifdef RPC_REMOTE_DISK
			if (pg_pwritev_rpc_local(fd, iov, iovcnt, offset) < 0)
#else
